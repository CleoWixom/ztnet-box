...
/// Read ZeroTier local auth token. Tries env var ZT_TOKEN first,
/// then falls back to the default authtoken.secret path.
...
/// Build app + check daemon reachable. Returns None to skip the test.
async fn app_or_skip() -> Option<axum::Router> {
    let token = zt_local_token()?;

    // Quick reachability probe before building a full app
    ...
    // Pass token directly via a temp file so AppState picks it up
    let tmp = tempfile::NamedTempFile::new().ok()?;
    std::fs::write(tmp.path(), &token).ok()?;
    cfg.zerotier.local.token_file = tmp.path().to_path_buf();
    // Keep tmp alive for the duration — leak intentionally (test process exits after)
    std::mem::forget(tmp);
...
async fn local_node_status_returns_address() {
    let Some(app) = app_or_skip().await else { return };
    ...
    assert_eq!(body["address"].as_str().unwrap().len(), 10, "ZT address is 10 hex chars");
...
async fn local_peers_returns_array() {
    ...
    // Every peer must have address, role and latency
...
async fn local_network_join_and_leave() {
    ...
    // ZeroTier's public "Earth" network — safe to join/leave in tests.
    // The node will be unauthorized but the join/leave API calls succeed.
    let test_net = std::env::var("ZT_TEST_NETWORK")
        .unwrap_or_else(|_| "8056c2e21c000001".to_string());
    ...
    let found = nets.as_array().unwrap().iter().any(|n| n["id"].as_str() == Some(&test_net));
    ...
    let r = get(&app, "/api/local/networks").await;
    let nets = body_json(r).await;
    let still_there = nets.as_array().unwrap().iter().any(|n| n["id"].as_str() == Some(&test_net));
...
async fn controller_network_crud() {
    ...
    // Get our node address to build a valid network ID prefix
    let r = get(&app, "/api/local/status").await;
    ...
    let net_id = net["id"].as_str().expect("created network must have id").to_string();
    assert_eq!(net_id.len(), 16, "network ID must be 16 chars");
    assert!(net_id.starts_with(&address), "network ID must start with node address");
    ...
    assert!(ids.contains(&net_id.as_str()), "created network must appear in list");
...
async fn controller_members_crud() {
    ...
    let address = match status["address"].as_str() {
        Some(a) => a.to_string(),
        None => { eprintln!("[SKIP] No node address"); return; }
    };

    // Create a network to add members to
    let r = post_json(&app, "/api/local/controller/networks",
        r#"{"name":"ztnet-test-members","private":true}"#).await;
    if r.status() == StatusCode::BAD_GATEWAY {
    ...
    // Use a dummy node ID (our own address is valid for member operations)
    ...
    let r = put_json(&app, &member_path,
        r#"{"authorized":true,"activeBridge":false}"#).await;
    ...
    // List members
    let r = get(&app, &format!("/api/local/controller/networks/{net_id}/members")).await;
    assert_eq!(r.status(), StatusCode::OK);
    let members = body_json(r).await;
    assert!(members.is_array() || members.is_object(), "members must be array or object");
...
async fn local_config_roundtrip() {
    ...
    // Write it back unchanged — must succeed
    let r = put_json(&app, "/api/local/config",
        &serde_json::to_string(&current).unwrap()).await;
    assert!(
