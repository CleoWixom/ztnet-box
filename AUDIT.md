...
## 17. MEDIUM — `Modal.prompt` вызывается с optional chaining без реализации
...
```javascript
// relay.js, _removeRemote()
const pass = await Modal.prompt?.('SSH password or leave empty to use key:') ?? null;
```

`Modal.prompt?.()` использует optional chaining — это означает, что автор знал о возможном отсутствии метода. Если `Modal.prompt` не определён (что нужно проверить в `modal.js`), функция тихо возвращает `null`, и операция удаления relay запускается без пароля.

Это особенно проблематично: операция удаления требует SSH-подключения с аутентификацией, и тихая передача `null` приведёт к ошибке на уровне SSH, которая не будет понятна пользователю.

### Рекомендация

Реализовать `Modal.prompt` в `modal.js` или заменить на явный inline-инпут в UI. Не использовать optional chaining для критических операций — лучше явный `if (!Modal.prompt) throw new Error('...')`.
...
| `GET /api/local/networks/:id/localconf` | Per-network local.conf | Не используется |
...
| # | Приоритет | Компонент | Проблема |
|---|-----------|-----------|----------|
| 1 | 🔴 Critical | Build | `log-panel.js` не включается в бандл → `LogPanel is not defined` |
| 2 | 🔴 High | Rust | Rate limiter не работает (permit сразу дропается) |
| 3 | 🔴 High | JS ↔ API | POST вместо PUT для update controller network → 405 |
| 4 | 🔴 High | Security | SSH: `StrictHostKeyChecking=no` |
| 5 | 🔴 High | Security | Docker install via `curl | sh` |
| 6 | 🔴 High | Security | SSH-пароль в plain JSON по HTTP |
| 7 | 🔴 High | Frontend | Нет мобильной адаптации (`@media` queries) |
| 8 | 🔴 High | Rust | `rand_byte()` читает `/dev/urandom` по 1 байту; fallback — константа |
| 9 | 🟡 Medium | Rust | `update_token` уничтожает оригинальный UUID |
| 10 | 🟡 Medium | JS | N+1 последовательных запросов в controllers-networks |
| 11 | 🟡 Medium | CSS/JS | Хардкод CSS-значений в `log-panel.js` (несовместимые переменные) |
| 12 | 🟡 Medium | Rust | Неиспользуемые тип-алиасы `PhysNetStateArc`, `RelayRemoteState` |
| 13 | 🟡 Medium | Frontend | `PeersPage` inline в `shell.html`, не загружает данные при прямом переходе |
| 14 | 🟡 Medium | Rust | Поле `zt_network_id` содержит имя интерфейса, а не ID сети |
| 15 | 🟡 Medium | Frontend | Нет кнопки toggle sidebar (мобильная и десктоп UX) |
| 16 | 🟡 Medium | Frontend | `metricstoken_file` не настраивается через UI |
| 17 | 🟡 Medium | Frontend | `Modal.prompt?.()` — тихий fail при отсутствии метода |
| 18 | 🟡 Medium | Rust | bridge/physnet/relay state in-memory, теряется при перезапуске |
| 19 | 🟡 Medium | Rust | Внешние зависимости от `ssh`/`sshpass` CLI-утилит |
| 20 | 🟢 Low | Security | CSP: `unsafe-inline` + `connect-src *` |
| 21 | 🟢 Low | Security | `danger_accept_invalid_certs` при не-localhost api_url |
| 22 | 🟢 Low | Frontend | Дублирование функции `_esc()` |
| 23 | 🟢 Low | Frontend | 8 backend-эндпоинтов без UI-покрытия |
| 24 | 🟢 Low | Rust | `#[allow(clippy::derivable_impls)]` вместо `#[derive(Default)]` |
| 25 | 🟢 Low | Rust | `network_id` не передаётся в `ExitNodeManager::enable()` |
